...
function applyAllowDenyActions(srcResourceAction, resourceActions, options = {
    ...
    let evalDenyAction = (_.isBoolean(options.evaluateDenyActions)) ? options.evaluateDenyActions : true;
    let evalAllowAction = (_.isBoolean(options.evaluateAllowActions)) ? options.evaluateAllowActions : true;
    ...
    for (let resourceName in tmpResourceActions) {
        ...
        if (evalAllowAction) {
            skipAllowActions = (options.evaluateAllowActions) ? filterActionInTarget(srcResourceAction.allow, tmpResourceActions[resourceName].allow) : [];
            tmpResourceActions[resourceName].allow.push(...srcResourceAction.allow);
            let tmpSet = new Set(tmpResourceActions[resourceName].allow);
            tmpResourceActions[resourceName].allow = [...tmpSet];
        ...
        if (evalDenyAction) {
            ...
            let tmpSet = new Set(tmpResourceActions[resourceName].deny);
            tmpResourceActions[resourceName].deny = [...tmpSet];
